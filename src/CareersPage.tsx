...
// ── Small reusable form modal shell ───────────────────────────────────────────
...
const QuickFormModal = ({ title, subtitle, contextLabel, onClose, onSubmit }: QuickFormModalProps) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <motion.div
        ...
        className="relative bg-white rounded-[2.5rem] w-full max-w-lg shadow-2xl overflow-hidden max-h-[90vh] overflow-y-auto z-10"
      >
        {/* Header */}
        <div className="relative bg-gradient-to-r from-[#0057FF] to-[#00C4CC] p-8 text-white">
          <button
            type="button"
            onClick={onClose}
            className="absolute top-5 right-5 w-9 h-9 bg-white/20 hover:bg-white/30 rounded-full flex items-center justify-center transition-colors z-20 cursor-pointer pointer-events-auto"
            aria-label="Close"
          >
            <X size={16} />
          </button>
          ...
          <h2 className="text-xl font-heading font-black leading-snug pr-10">{title}</h2>
          <p className="text-blue-100 text-[11px] font-medium mt-1 opacity-80">{subtitle}</p>
        </div>
        ...
        {submitted ? (
          <div className="p-8 flex flex-col items-center text-center gap-4">
            ...
            <h3 className="font-heading font-black text-slate-900 text-lg">Request Sent!</h3>
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-5">
            <p className="text-slate-500 text-xs font-medium leading-relaxed">
              Fill in a few details and we'll connect with you right away.
            </p>
            ...
            <div className="space-y-2">
              ...
              <textarea
                name="message"
                rows={3}
                placeholder="Tell us a bit about your background or what you're looking for..."
                className="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-medium text-slate-900 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:bg-white transition-all resize-none"
              />
            </div>
            <button
              type="submit"
              className="w-full py-4 bg-gradient-to-r from-[#0057FF] to-[#00C4CC] text-white rounded-2xl font-black text-sm uppercase tracking-widest flex items-center justify-center gap-2 hover:scale-[1.02] active:scale-95 transition-all shadow-lg shadow-blue-500/20 cursor-pointer"
            >
              <Send size={16} /> Submit
            </button>
          </form>
...
const JobApplyModal = ({ role, onClose }: { role: Role; onClose: () => void }) => {
  const [submitted, setSubmitted] = useState(false);
  ...
  const handleSubmit = (e: FormEvent<HTMLFormElement>) => {
    ...
    window.open(`https://wa.me/19457329000?text=${encodeURIComponent(waText)}`, '_blank');
    setSubmitted(true);
  };
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      >
        ...
        <div className="relative bg-gradient-to-r from-[#0057FF] to-[#00C4CC] p-8 text-white">
          ...
          <h2 className="text-xl font-heading font-black mb-1 leading-snug pr-10">{role.title}</h2>
        ...
        {submitted ? (
          <div className="p-8 flex flex-col items-center text-center gap-4">
            ...
            <p className="text-slate-500 text-sm font-medium leading-relaxed">
              Your application for <strong>{role.title}</strong> has been submitted.
          ...
          <form onSubmit={handleSubmit} className="p-8 space-y-5">
            <p className="text-slate-500 text-xs font-medium leading-relaxed">
              Fill in a few quick details and we'll receive your application instantly.
            </p>
            <div className="space-y-2">
              <label className="text-[11px] font-black text-slate-900 uppercase tracking-widest">
                Full Name <span className="text-red-500">*</span>
...
// ── Data ───────────────────────────────────────────────────────────────────────
const cultureItems = [
  { icon: <Lightbulb size={20} />, title: 'Innovative Projects', desc: 'Work on real-world impactful projects.' },
  { icon: <TrendingUp size={20} />, title: 'Growth & Learning', desc: 'Continuous learning and self-development.' },
  { icon: <Users size={20} />, title: 'Great Environment', desc: 'Collaborative, inclusive and supportive culture.' },
];
...
// ── Main Page ──────────────────────────────────────────────────────────────────
type ModalType = 'job' | 'differentRole' | 'newRole' | null;

export const CareersPage = ({ onNavigate: _onNavigate, onTalkToCounselor: _onTalkToCounselor }: CareersPageProps) => {
  ...
  const sendToWhatsApp = (prefix: string, data: { name: string; phone: string; email: string; message: string }) => {
    const waText =
      `${prefix}\n\n` +
      `👤 *Name:* ${data.name}\n` +
      `📱 *Phone:* ${data.phone}\n` +
  ...
  return (
    <div className="bg-white">
      {/* Job apply modal */}
      <AnimatePresence>
        {selectedRole && <JobApplyModal role={selectedRole} onClose={closeAll} />}
      </AnimatePresence>

      {/* "Different role" modal */}
      <AnimatePresence>
        {activeModal === 'differentRole' && (
          <QuickFormModal
            title="Apply for a Different Role"
            subtitle="Don't see your fit? Share your profile with us."
            contextLabel="Open Application"
            onClose={closeAll}
            onSubmit={(data) =>
              sendToWhatsApp('📋 *Open Application – Different Role*\nThe applicant is interested in a role not listed.', data)
            }
          />
        )}
      </AnimatePresence>

      {/* "New role request" modal */}
      <AnimatePresence>
        {activeModal === 'newRole' && (
          <QuickFormModal
            title="Request a New Job Requirement"
            subtitle="Have a specific role in mind? Let us know."
            contextLabel="Job Requirement Request"
            onClose={closeAll}
            onSubmit={(data) =>
              sendToWhatsApp('🔔 *New Job Requirement Request from itcareer.co*\nThe applicant wants to discuss a new or custom role.', data)
            }
          />
        )}
      </AnimatePresence>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10">
                {cultureItems.map((item) => (
                  ...
                  >
                    ...
                    <h4 className="font-heading font-black text-slate-900 text-[12px] leading-tight">{item.title}</h4>
              ...
              <div className="flex flex-wrap items-center gap-4">
                <button
                  onClick={() => { const el = document.getElementById('open-positions'); el?.scrollIntoView({ behavior: 'smooth' }); }}
                ...
                <button
                  onClick={() => setActiveModal('differentRole')}
            ...
            {openRoles.map((role, i) => (
              ...
              >
                <div className="flex items-start justify-between flex-wrap gap-2">
                  <div>
                    <h3 className="font-heading font-black text-slate-900 text-base md:text-lg leading-tight mb-1">{role.title}</h3>
                ...
                <button
                  onClick={() => { setSelectedRole(role); setActiveModal('job'); }}
        ...
        <div className="section-container">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            >
              ...
              <h3 className="font-heading font-black text-slate-900 text-lg">I'm a Great Fit for Another Role</h3>
              <p className="text-slate-500 text-[13px] font-medium leading-relaxed">
                Don't see a role that matches your skills? Share your profile with us and we'll consider you for future opportunities.
              </p>
              <button
                onClick={() => setActiveModal('differentRole')}
            ...
            >
              ...
              <h3 className="font-heading font-black text-slate-900 text-lg">Request a New Job Requirement</h3>
              <p className="text-slate-500 text-[13px] font-medium leading-relaxed">
                Have a specific role in mind? Let us know what you're looking for and we'll work together to find the right match.
              </p>
              <button
        ...
        <div className="section-container">
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-14">
            {whyJoin.map((item, i) => (
              ...
              >
                <div className="w-9 h-9 bg-blue-50 rounded-xl flex items-center justify-center text-[#0057FF]">{item.icon}</div>
